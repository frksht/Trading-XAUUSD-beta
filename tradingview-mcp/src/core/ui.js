 ...
 */
import { evaluate, evaluateAsync, getClient, safeString, safeObject, rateLimit } from '../connection.js';

export async function click({ by, value }) {
  rateLimit('ui.click', 10, 1000);
  const escapedValue = safeString(value);
  const escapedBy = safeString(by);
  const result = await evaluate(`
    (function() {
      var by = ${escapedBy};
      var value = ${escapedValue};
      var safeCSS = function(v) { return v.replace(/"/g, '\\\\"').replace(/[\\\\\\]]/g, ''); };
      var el = null;
      if (by === 'aria-label') el = document.querySelector('[aria-label="' + safeCSS(value) + '"]');
      else if (by === 'data-name') el = document.querySelector('[data-name="' + safeCSS(value) + '"]');
      else if (by === 'text') {
      ...
      } else if (by === 'class-contains') el = document.querySelector('[class*="' + safeCSS(value) + '"]');
  ...
  if (isBottomPanel) {
    ...
    const result = await evaluate(`
      (function() {
        ...
        var panel = ${safeString(panel)};
        var widgetName = ${safeString(widgetName)};
        var action = ${safeString(action)};
    ...
    const result = await evaluate(`
      (function() {
        var dataName = ${safeString(sel.dataName)};
        var ariaLabel = ${safeString(sel.ariaLabel)};
        var action = ${safeString(action)};
...
export async function hover({ by, value }) {
  const coords = await evaluate(`
    (function() {
      ...
      var el = null;
      if (by === 'aria-label') {
        el = document.querySelector('[aria-label="' + safeCSS(value) + '"]');
        if (!el) el = document.querySelector('[aria-label*="' + safeCSS(value) + '"]');
      }
      else if (by === 'data-name') el = document.querySelector('[data-name="' + safeCSS(value) + '"]');
      else if (by === 'text') {
