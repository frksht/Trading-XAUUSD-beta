...
import { safeString, requireFinite, safeObject, rateLimit, resetRateLimits } from '../src/connection.js';
...
describe('safeObject() — object sanitization', () => {
  it('passes through null and undefined', () => {
    assert.equal(safeObject(null), null);
    assert.equal(safeObject(undefined), undefined);
  });

  it('passes through primitives', () => {
    assert.equal(safeObject(42), 42);
    assert.equal(safeObject(true), true);
  });

  it('sanitizes strings', () => {
    assert.equal(safeObject('hello'), 'hello');
  });

  it('rejects strings exceeding max length', () => {
    assert.throws(() => safeObject('x'.repeat(10001)), /exceeds maximum length/);
  });

  it('sanitizes nested objects', () => {
    const obj = { a: { b: { c: 'test' } } };
    assert.deepEqual(safeObject(obj, 5), obj);
  });

  it('rejects excessive nesting depth', () => {
    const deep = { a: { b: { c: { d: { e: { f: 'too deep' } } } } } };
    assert.throws(() => safeObject(deep, 3), /exceeds maximum nesting depth/);
  });

  it('rejects invalid object keys', () => {
    assert.throws(() => safeObject({ 'bad-key': 'value' }), /Invalid object key/);
    assert.throws(() => safeObject({ 'alert(1)': 'value' }), /Invalid object key/);
  });

  it('sanitizes arrays recursively', () => {
    const arr = [{ name: 'test', value: 'safe' }];
    assert.deepEqual(safeObject(arr), arr);
  });

  it('allows valid object keys', () => {
    const obj = { _validKey: 1, $alsoValid: 2, valid123: 3 };
    assert.deepEqual(safeObject(obj), obj);
  });
});

// ── rateLimit() ───────────────────────────────────────────────────────────

describe('rateLimit() — rate limiting', () => {
  it('allows calls within limit', () => {
    resetRateLimits();
    for (let i = 0; i < 5; i++) rateLimit('test', 5, 1000);
  });

  it('throws when limit exceeded', () => {
    resetRateLimits();
    for (let i = 0; i < 5; i++) rateLimit('test', 5, 1000);
    assert.throws(() => rateLimit('test', 5, 1000), /Rate limit exceeded/);
  });

  it('separate keys have independent limits', () => {
    resetRateLimits();
    for (let i = 0; i < 5; i++) rateLimit('key1', 5, 1000);
    rateLimit('key2', 5, 1000); // Should not throw
  });
  ...
  it('resetRateLimits clears all state', () => {
    ...
    for (let i = 0; i < 5; i++) rateLimit('test', 5, 1000);
    resetRateLimits();
    rateLimit('test', 5, 1000); // Should not throw after reset
  });
});
